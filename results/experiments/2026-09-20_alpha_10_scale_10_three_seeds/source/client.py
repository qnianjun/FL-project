...
def create_dirichlet_partition(dataset, num_clients, alpha):
    ...
    for class_id in range(num_classes):
        ...
        # Floating-point sums can fall just below 1 and drop the last sample.
        proportions[-1] = len(class_indices)
...
class FlowerClient(
    ...
    def __init__(self, cid, datasets_pair=None):
        ...
        trainset, testset = datasets_pair if datasets_pair is not None else load_data()
    ...
    def fit(self, parameters, config):
        ...
        update_norm = float(np.sqrt(sum(
            np.sum((new.astype(np.float64) - old) ** 2)
            for new, old in zip(new_parameters, old_parameters)
        )))
        ...
        return (
            ...
            len(self.trainset),
            {
                ...
                "sent_update_norm": float(np.sqrt(sum(
                    np.sum((new.astype(np.float64) - old) ** 2)
                    for new, old in zip(new_parameters, old_parameters)
                ))),
            }
