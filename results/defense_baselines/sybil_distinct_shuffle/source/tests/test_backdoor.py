...
class BackdoorTests(unittest.TestCase):
    ...
    def test_selection_deterministic_and_fixed_count(self):
        ...
        self.assertFalse(
            np.array_equal(selected, poison_selection(101, 0.2, 2043))
        )
    ...
    def test_matched_workers_shuffles_and_attacker_isolation(self):
        partitions = [
            list(range(cid * 20, (cid + 1) * 20)) for cid in range(5)
        ]
        ...
        self.assertEqual(
            identity(clean, partitions), identity(backdoor, partitions)
        )
    ...
    def test_asr_excludes_original_targets_and_preserves_clean_test(self):
        ...
        self.assertTrue(
            all(self.labels[i] != 0 for i in triggered.dataset.indices)
        )
        self.assertTrue(
            all(triggered[i][1] == 0 for i in range(len(triggered)))
        )
        # Constant target predictions: 100% ASR, 10% clean accuracy.
        model = client.Net()
        logits = torch.zeros(1, 10)
        logits[0, 0] = 1
        with patch.object(
            model, "forward", side_effect=lambda x: logits.repeat(len(x), 1)
        ):
            _, asr = client.test(model, DataLoader(triggered, batch_size=32))
            _, clean = client.test(
                model, DataLoader(self.dataset, batch_size=32)
            )
