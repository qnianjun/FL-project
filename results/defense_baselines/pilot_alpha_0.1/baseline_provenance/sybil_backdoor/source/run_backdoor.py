...
def apply_trigger(image, settings=SETTINGS):
    ...
    result[:, row : row + size, column : column + size] = (
        settings.trigger_value
    )
    return result
...
class TriggeredDataset(Dataset):
    """Transform copies on read; preserve the underlying dataset."""
    ...
    def __getitem__(self, index):
        ...
        if index in self.positions:
            return (
                apply_trigger(image, self.settings),
                self.settings.target_label,
            )
        return image, label
        ...
        "statistics": {
            metric: {
                ...
                "sample_std": statistics.stdev(
                    row[metric] for row in differences
                ),
            }
            for metric in metrics
        },
    ...
    with (folder / "results.csv").open("w", newline="") as handle:
        ...
        for round_number in range(1, SETTINGS.rounds + 1):
            ...
            for worker in workers:
                ...
                updates.append(
                    (worker.get_parameters({}), len(worker.trainset))
                )
...
def main():
    ...
    parser.add_argument(
        "--run", action="store_true", help="Start pilot training"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    for seed in SETTINGS.seeds:
        ...
        for attack in (False, True):
            ...
            workers, selected = make_workers(
                datasets_pair, partitions, seed, attack
            )
            ...
            if paired_identity is None:
                ...
                assert (
                    current_identity == paired_identity
                ), "Pair is not matched"
            ...
            write_json(
                folder / "config.json",
                {
                    ...
                    **current_identity,
                    ...
                    "client_label_counts": [
                        np.bincount(
                            np.asarray(datasets_pair[0].targets)[part],
                            minlength=10,
                        ).tolist()
            ...
            final.append(
                train_condition(workers, asr_loader, folder, seed, mode)
            )
            assert dataset_hashes(datasets_pair) == original_hashes
    if args.run:
