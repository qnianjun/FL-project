...
Five clients, 10 rounds, one local epoch, SGD learning rate 0.01, batch size 32, MNIST, alpha 10.0, client 0 update scale 10. Seeds: 42, 43, 44.

| Seed | Clean final accuracy | Poisoned final accuracy | Drop (percentage points) |
| --- | --- | --- | --- |
| 42 | 90.92% | 9.85% | 81.07 |
| 43 | 90.85% | 46.44% | 44.41 |
| 44 | 90.99% | 28.33% | 62.66 |

- Clean mean ± sample SD: **90.92% ± 0.07 percentage points**.
- Poisoned mean ± sample SD: **28.21% ± 18.30 percentage points**.
- Paired accuracy drop: **62.71 ± 18.33 percentage points**.
...
- Clean final loss: **0.3240 ± 0.0020**.
- Poisoned final loss: **10.8898 ± 9.1500**.
- Paired loss damage: **10.5657 ± 9.1482**.
...
Client 0 has [14398, 9733, 12846] samples across seeds, respectively (FedAvg weights clients by sample count). This changes the attacker's aggregation weight and may help explain the varying impact, but these runs do not isolate that effect.
...
```bash
.venv/bin/python run_matched.py --alpha 10.0 --scale 10 --seeds 42 43 44 --rounds 10 --output results/experiments/NEW_RUN
