...
def main():
    ...
    if not math.isfinite(args.alpha) or not math.isfinite(args.scale) or args.alpha <= 0 or args.scale <= 0 or args.rounds < 1:
    ...
    source.mkdir()
    for name in ['client.py', 'run_balanced.py', 'balanced_partition.py', 'config.json']:
    ...
    write_json(args.output / 'environment.json', {
        ...
        'packages': {name: importlib.metadata.version(name) for name in ['torch', 'torchvision', 'numpy', 'flwr']},
    ...
    write_json(args.output / 'dataset_hashes.json', {
        str(p): hashlib.sha256(p.read_bytes()).hexdigest()
        for p in sorted(Path('data/MNIST/raw').iterdir()) if p.is_file() and p.suffix != '.gz'
    })
    ...
    for seed in args.seeds:
        ...
        for attack in (False, True):
            ...
            workers = [BalancedClient(cid, datasets_pair, partitions, seed) for cid in range(5)]
            initial = workers[0].get_parameters({})
            indices = [np.asarray(w.trainset.indices, dtype=np.int64) for w in workers]
            all_indices = np.concatenate(indices)
            assert np.array_equal(np.sort(all_indices), np.arange(len(datasets_pair[0]))), 'Partition must cover all training samples exactly once'
            identity = {'initial_parameters_sha256': digest_arrays(initial),
            ...
            np.savez_compressed(folder / 'partitions.npz', **{f'client_{i}': value for i, value in enumerate(indices)})
            write_json(folder / 'config.json', {
                'seed': seed, 'alpha': args.alpha, 'enable_poison': attack,
                'partition_method': 'capacity_constrained_dirichlet_v1',
                'samples_per_client': len(datasets_pair[0]) // 5,
                'poison_client': 0, 'poison_scale': args.scale, 'rounds': args.rounds,
                'num_clients': 5, 'batch_size': 32, 'local_epochs': 1, 'optimizer': 'SGD', 'learning_rate': 0.01,
                'shuffle_seeds': [seed + 1000 + i for i in range(5)],
                'client_sample_counts': [len(w.trainset) for w in workers],
                'client_label_counts': [np.bincount(np.asarray(datasets_pair[0].targets)[i], minlength=10).tolist() for i in indices],
                **identity,
            })
            parameters = initial
            with (folder / 'results.csv').open('w', newline='') as result_file, (folder / 'updates.csv').open('w', newline='') as update_file:
                ...
                update_writer.writerow(['round', 'client', 'update_norm', 'sent_update_norm', 'scale_ratio'])
                for round_number in range(1, args.rounds + 1):
                    ...
                    for worker in workers:
                        ...
                        assert math.isclose(sent, expected_scale * norm, rel_tol=1e-5, abs_tol=1e-7)
                        update_writer.writerow([round_number, worker.cid, norm, sent, sent / norm if norm else ''])
                    ...
                    result_file.flush()
                    update_file.flush()
                    print(f'seed={seed} {mode} round={round_number}/{args.rounds} accuracy={accuracy:.4f} loss={loss:.4f}', flush=True)
            final.append({'seed': seed, 'condition': mode, 'accuracy': accuracy, 'loss': loss})
    ...
    summary = {'runs': final, 'clean_mean_accuracy': statistics.mean(clean),
               'clean_sample_std': statistics.stdev(clean), 'poisoned_mean_accuracy': statistics.mean(poisoned),
               'poisoned_sample_std': statistics.stdev(poisoned), 'paired_drops_percentage_points': drops,
               'mean_drop_percentage_points': statistics.mean(drops), 'drop_sample_std_percentage_points': statistics.stdev(drops)}
